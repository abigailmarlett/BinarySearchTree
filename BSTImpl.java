...
public class BSTImpl implements BST {
   ...
   //====================================================================================
    ...
    private boolean isFullBT_r(Node c){
        boolean result = false;
        if (c.getLeft() != null && c.getRight() == null) {return false;}
        if (c.getRight() != null && c.getLeft() == null) {return false;}
        if(c.getLeft() == null && c.getRight() == null){
           result =  true;
       }
        if(c.getLeft() != null && c.getRight() != null){
            result = isFullBT_r(c.getLeft()) && isFullBT_r(c.getRight());
        }
       return result;
    }
    ...
    //    }
      ...
      return merge_r(c, counter);  // Dummy return statement.  Remove when you implement!
    }
    private int merge_r(Node c, int counter){
        if (this.contains(c.getValue()) == false){
            counter++;
        }
        this.insert(c.getValue());
        if (c.getLeft() != null){
            counter = merge_r(c.getLeft(), counter);
        }
        if (c.getRight() != null){
            counter = merge_r(c.getRight(), counter);
        }
    ...
    public int getMaxLeafHeightDiff () {
        ...
        return getMaxLeafHeightDiff_max_r(this.root) - getMaxLeafHeightDiff_min_r(this.root) ;// Dummy return statement.  Remove when you implement!
    }

    private int getMaxLeafHeightDiff_max_r(Node c) {
        // private inner "helper" method with different signature
        // this helper method uses recursion to traverse
        // and process the recursive structure of the tree of cells
        if (c==null) return -1;
        int lht = getMaxLeafHeightDiff_max_r(c.getLeft());
        int rht = getMaxLeafHeightDiff_max_r(c.getRight());
        return Math.max(lht,rht) + 1;
    }
    private int getMaxLeafHeightDiff_min_r(Node c) {
        // private inner "helper" method with different signature
        // this helper method uses recursion to traverse
        // and process the recursive structure of the tree of cells
        if (c==null) return -1;
        int lht = getMaxLeafHeightDiff_min_r(c.getLeft());
        int rht = getMaxLeafHeightDiff_min_r(c.getRight());
